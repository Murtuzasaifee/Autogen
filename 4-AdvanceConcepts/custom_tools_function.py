...
web_scraper_agent = AssistantAgent(
    ...
    model_client=openai_client,
    system_message="You are a helpful assistant that can fetch and convert webpages to markdown using the fetch_webpage tool.",
    tools=[fetch_webpage],
...
task = "Search for the best deal for iphone16 from amazon.ae"

# Run the agent
async def main():
    # result = await reverse_agent.run(task=task)
    result = await web_scraper_agent.run(task=task)
